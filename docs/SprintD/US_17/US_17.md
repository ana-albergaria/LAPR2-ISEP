...
|                                                                                      |... knowing in which Clinical Analysis Laboratory the test should be registered?| ClinicalAnalysisLaboratoryStore    | IE: ClinicalAnalysisLaboratoryStore stores all of the Clinical Analysis Laboratory objects. |
|                                                                                      |... Getting client to be associated from TIN number?                            | ClientStore                        | IE: The client store knows it's clients tin numbers.|
|                                                                                      |... Getting TestType associated with given TestType code?                       | ClinicalAnalysisLaboratory         | IE: The identified CAL in step 3 knows which types of tests it provides.|
|                                                                                      |... Getting the Parameters associated with given parameters codes?              | ParametersStore                    | IE: The Parameter Store knows all existing parameters.|
| Step 4: shows the number of clinical tests to be imported\n and requests confirmation|... validating the data locally (e.g.: mandatory vs. non-mandatory data)?       | Test                               | IE: Knows its own data.|              
|                                                                                      |... validating the data globally (e.g.: duplicated)?				            | TestStore                          | IE: Knows all Test objects.|   
| Step 5: confirms the data                                                            |... Saving the created test?	                                                | TestStore                          | IE: Records/adopts all the Test objects.|
| Step 6: informs operation success                                                    | ImportTestUI                                                                   | IE: responsible for user interaction                 |   
